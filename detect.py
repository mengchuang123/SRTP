...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #Get face detect
        # img_cpy = Image.fromarray(np.transpose(im0s[0], (2,1,0)), mode='RGB')
        # print(np.array(im0s).shape)
        img_cpy = Image.fromarray(cv2.cvtColor(im0s[0], cv2.COLOR_BGR2RGB))
        # cv2.imshow('',im0s[0])
        # img_cpy.show()
        print(img_cpy)
        boxes, _ = mtcnn.detect(img_cpy)

        #进行人脸比对
        img_aligned,pro =  mtcnn(img_cpy, return_prob=True)
        img_list=[]
        ...
        # im0s[0]= img_cpy
        im0s[0] = cv2.cvtColor(np.array(img_cpy), cv2.COLOR_RGB2BGR)
        # print(Image.fromarray(img[0], mode='RGB'))
        #print(img.shape)
        count +=1
        if count > 20:
            flag = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--names', type=str, default='data/train/names.names', help='*.names path')
